...
from customasr_datamodule import CustomAsrDataModule
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        if((math.isinf(loss_info["ctc_loss"]) or math.isinf(loss_info["att_loss"]) or math.isinf(loss_info["loss"]))):
            print("Inf loss. Disgard this batch...")
            messed_up.append(batch)
            #loss_info["ctc_loss"] = 0
            #continue
            # summary stats
                      
            #loss_info["att_loss"] = 0
            #loss_info["loss"] *= 0
        ...
        optimizer.zero_grad()
        #if(not((math.isinf(loss_info["ctc_loss"]) or math.isinf(loss_info["att_loss"]) or math.isinf(loss_info["loss"])))):
        loss.backward()
        clip_grad_norm_(model.parameters(), 5.0, 2.0)
        optimizer.step()

        if batch_idx % params.log_interval == 0:
            #logging.info(f"Batch {batch}, Loss {loss_info}")
            logging.info(
    ...
    joblib.dump(messed_up, "/home/mselim/messedup_files")
    loss_value = tot_loss["loss"] / tot_loss["frames"]
...
def run(rank, world_size, args):
    ...
    librispeech = CustomAsrDataModule(args)

    train_cuts = librispeech.train_clean_cuts()
    ...
    def remove_short_and_long_utt(c: Cut):
        ...
        # the threshold
        return (1.0 <= c.duration <= 20.0) and ((len(c.supervisions[0].text.split()) / c.duration) <= 2.5) and (len(c.supervisions[0].text) != 0)
